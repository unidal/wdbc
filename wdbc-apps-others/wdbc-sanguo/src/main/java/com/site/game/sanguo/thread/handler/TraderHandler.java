...
public class TraderHandler implements ThreadHandler, LogEnabled {
	...
	private void checkBound(ThreadContext ctx, TraderTask task) {
		double total = task.getLumber() + task.getClay() + task.getIron()
				+ task.getCrop();
	...
	private boolean doTrade(ThreadContext ctx, TraderTask task)
		...
		try {
			String result = ThreadHelper.executeRequest(session,
					m_tradeRequest, true);

			if (result.indexOf("资源开始运输") > 0) {
				m_logger.info("Shipping resource(" + task.getLumber() + ","
						+ task.getClay() + "," + task.getIron() + ","
						+ task.getCrop() + ") from "
						+ task.getFromFarm().getVillageName() + "("
						+ task.getFromFarm().getX() + ","
						+ task.getFromFarm().getY() + ") to "
						+ task.getToFarm().getVillageName() + "("
						+ task.getToFarm().getX() + ","
						+ task.getToFarm().getY() + ")");
				return true;
