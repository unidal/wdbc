   ...
   public void doFilter(WdbcResult result) {
      ...
      for (int row = 0; row < rows; row++) {
         ...
         if (contact != null && contact.length() > 0) {
            if (contact.startsWith("��ϵ��")) {
               String phone = contact.substring("��ϵ��".length());
            ...
            } else if (contact.startsWith("�ʼ���")) {
               email = contact.substring("�ʼ���".length());
         ...
         if (header != null && header.length() > 0) {
            int pos1 = header.indexOf("���:�0�2");
            int pos2 = header.indexOf("λ��:�0�2");

            if (pos1 >= 0 && pos2 > 0) {
               category = header.substring("���:�0�2".length(), pos2).trim();
               district = header.substring(pos2 + "λ��:�0�2".length()).trim();
            }
         }
