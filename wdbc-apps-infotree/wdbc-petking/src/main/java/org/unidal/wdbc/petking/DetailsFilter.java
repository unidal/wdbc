package org.unidal.wdbc.petking;
...
public class DetailsFilter extends DefaultWdbcFilter {
	...
	public void doFilter(WdbcResult result) {
		...
		for (String part : parts) {
			...
			if (part.startsWith("房型：") || part.startsWith("租金：")
					|| part.startsWith("面积：")) {
				// ignore it
			} else {
		...
		result.clear();
		result.addValue("title", title.startsWith("标题：") ? title.substring(3)
				: title);
		result.addValue("body", content);
		result.addValue("contact", contactBuf.toString());
		result.addValue("category",
				category.length() > 5 ? category.substring(5) : category);
